...
/* ---------- Clint (Altori): CALL/0DTE/@premium + scissors exits ---------- */

test("Clint: '$NVDA 222.5 CALL 0DTE @1.00' (0DTE = today, @ premium)", () => {
  const s = parseSignal("$NVDA 222.5 CALL 0DTE @1.00", NOW);
  assert.equal(s.kind, "option");
  assert.equal(s.ticker, "NVDA");
  assert.equal(s.strike, 222.5);
  assert.equal(s.type, "C");
  assert.equal(s.premium, 1.0);
  assert.equal(s.expiry, "2026-07-16", "0DTE resolves to NOW's date");
  assert.ok(s.warnings.some(w => /expires today/i.test(w)), "must warn 0DTE expires today");
  assert.equal(s.occ, "NVDA260716C00222500");
});

test("Clint: NDTE adds N days", () => {
  assert.equal(parseSignal("SPY 500 PUT 2DTE @3.20", NOW).expiry, "2026-07-18");
});

test("Clint: scissors trim exit '✂️ $NVDA 222.5c +22%'", () => {
  const s = parseSignal("✂️ $NVDA 222.5c +22%", NOW);
  assert.equal(s.kind, "exit");
  assert.equal(s.action, "trim");
  assert.equal(s.ticker, "NVDA");
  assert.equal(s.strike, 222.5);
  assert.equal(s.type, "C");
  assert.equal(s.pct, 22);
});

test("Clint: second scissors trim at a higher gain", () => {
  assert.equal(parseSignal("✂️ $NVDA 222.5c +41%", NOW).pct, 41);
});

test("Clint entry is not misread as an exit", () => {
  assert.equal(parseSignal("$NVDA 222.5 CALL 0DTE @1.00", NOW).kind, "option");
});
