...
- **Status:** built + unit-tested (54 tests). Verified only against *mocked*
...
test/
  parse.test.mjs     33 tests: every format + refusals + year-roll + OCC
  alpaca.test.mjs    15 tests: order construction, exits, paper default
...
| `option` (DTE, Clint/Altori) | `$NVDA 222.5 CALL 0DTE @1.00` | `NDTE`→today+N days (0DTE warns "expires today"); `@`premium |
| `exit` | `NVDA 215c TRIMMED AT 44%` / `ALL OUT 226%` / `✂️ $NVDA 222.5c +22%` | ticker/strike/type, `action` trim/close, `pct`; scissors emoji = trim |
...
```bash
node --test test/         # full unit suite (54)
```
