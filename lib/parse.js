...
export function classify(text) {
  ...
  if (/\btrimmed\b|\ball out\b|\bal out\b|\bout at\b/i.test(text) || /✂|✄/.test(text)) return "exit";
...
function parseOptionsLine(rawLine, now) {
  ...
  // Keyword pass: AVG <num>, EXP <date>, NDTE (days-to-expiry), @<premium>.
  for (let i = 0; i < tokens.length; i++) {
    ...
    if ((w === "EXP" || w === "EXPIRY" || w === "EXPIRES" || w === "EXPIRATION") && !expiry) {
      const ex = matchExpiry(tokens[i + 1] || "");
      if (ex) { expiry = ex; consumed[i] = consumed[i + 1] = true; keyworded = true; }
      continue;
    }
    // 0DTE / 1DTE / 2DTE ... -> expiry = today + N days (0DTE = today).
    const dteM = tok.match(/^(\d+)DTE$/i);
    if (dteM && !expiry) {
      const days = parseInt(dteM[1], 10);
      const d = new Date(now);
      d.setUTCDate(d.getUTCDate() + days);
      expiry = { month: d.getUTCMonth() + 1, day: d.getUTCDate(), year: d.getUTCFullYear() };
      consumed[i] = true; keyworded = true;
      warnings.push(days === 0 ? "0DTE — this contract EXPIRES TODAY." : `${days}DTE — expires in ${days} day(s).`);
      continue;
    }
    // @1.00 / @.64 premium syntax.
    const atM = tok.match(/^@\$?(\d+(?:\.\d+)?|\.\d+)$/);
    if (atM && premium == null) {
      premium = parseFloat(atM[1]); consumed[i] = true; keyworded = true;
      continue;
    }
  }
